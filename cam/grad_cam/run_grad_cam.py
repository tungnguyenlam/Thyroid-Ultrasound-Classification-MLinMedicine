"""
cam/grad_cam/run_grad_cam.py
============================
Ensemble Grad-CAM visualisation for the Thyroid Ultrasound Classification project.
...
3.  For every image in the fixed test set it:
        a. Runs a forward + backward pass through each fold model to get
           the Grad-CAM heat-map for that fold.
        b. Normalises each fold's heat-map to [0,1] and averages them.
4.  Saves a 3-panel PNG per image under
        cam/grad_cam/outputs/<class>/<stem>_gradcam.png
    showing: original | Grad-CAM overlay | stand-alone heatmap.
...
----------------------------------
    python cam/grad_cam/run_grad_cam.py
    python cam/grad_cam/run_grad_cam.py --config config/config.yaml \\
                                         --target-class 1 \\
                                         --n-images 50 \\
                                         --alpha 0.45
"""
...
# ===========================================================================
#  Automatic target-layer resolver
# ===========================================================================
...
def get_target_layer(model: nn.Module, model_name: str) -> nn.Module:
    ...
    This is the layer whose activation maps and gradients are used to
    produce the Grad-CAM heat-map.  The choice below gives the best spatial
    resolution while still being semantically rich:

        res18 / res50   -> model.layer4        (final residual stage)
        densenet        -> model.features.denseblock4
...
class GradCAM:
    """
    Single-layer Grad-CAM via forward/backward hooks.

    Args:
        model        : PyTorch model already in eval mode.
        target_layer : The module to hook (e.g. model.layer4).
    """
    ...
    def __call__(
        ...
        Returns:
            cam          : (h, w) float32 heat-map in [0, 1] (spatial size of
                           the target layer output, NOT the input).
            used_class   : The class that was actually back-propagated.
        """
...
def get_test_paths_labels(cfg: dict) -> tuple[list, list]:
    """
    Reproduce the same deterministic test split used during training and return
    (list_of_image_paths, list_of_labels).
    """
    import numpy as np
...
def run_ensemble_gradcam(
    ...
    alpha       : float         = 0.45,
    out_dir     : str           = "cam/grad_cam/outputs",
) -> None:
    """
    Compute per-image ensemble Grad-CAM over the shared test set and save
    ...
    print(f"\n{'='*60}")
    print(f"  Ensemble Grad-CAM  |  model: {model_name}")
    ...
    print(f"[Grad-CAM] Test images to process: {total}\n")
    ...
    for idx in tqdm(range(total), desc="[Grad-CAM]", unit="img"):
        ...
        for m in models:
            ...
            gcam      = GradCAM(m, tgt_layer)
            cam_k, cls_k = gcam(input_t.clone(), target_class=target_class)
            gcam.remove_hooks()
        ...
        fig.suptitle(
            f"Grad-CAM  ({model_name}, {n_folds}-fold ensemble)  |  "
        ...
        save_path = os.path.join(out_dir, class_names[true_label], f"{stem}_gradcam.png")
        plt.savefig(save_path, dpi=150, bbox_inches="tight")
        plt.close(fig)

    print(f"\n[Grad-CAM] Done — {total} images saved under '{out_dir}'")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Ensemble Grad-CAM for Thyroid Ultrasound Classification.\n"
    ...
    parser.add_argument(
        "--out-dir",
        type=str, default="cam/grad_cam/outputs",
        help="Root output directory (default: cam/grad_cam/outputs)",
    )
